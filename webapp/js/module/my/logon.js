 ...
 * @Last Modified time: 2017-11-09 16:53:41
 */
...
fjw.webapp.logon = {
    ...
    getVCode: function () {
        ...
        }, validateResult = _util.validate(fromDataCode.Phone, 'require');
    ...
    submit: function () {
        ...
        if (validateResult.status && !$('#logonbtn').hasClass('disabled')) {
            _user.register(JSON.stringify(req), function (res) {
                ...
                // 注册成功 保存状态 cookie
                _util.storage.setItem('F.token', user.token, { path: '/' });
                _util.storage.setItem('F.phone', user.member.phone, { path: '/' });
                _util.storage.setItem('F.avator', user.member.headPhoto, { path: '/' });

                _util.storage.setItem('f_ui_cache', res);
